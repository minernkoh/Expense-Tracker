...
class DataController: ObservableObject {
    ...
    private static let logger = Logger(subsystem: "com.rafaelsoh.dime", category: "DataController")
...
////        }
    ...
    var addedTransaction: Bool {
        get {
            (UserDefaults(suiteName: "group.com.rafaelsoh.dime") ?? .standard).bool(forKey: "newTransactionAdded")
        }

        set {
            (UserDefaults(suiteName: "group.com.rafaelsoh.dime") ?? .standard).set(newValue, forKey: "newTransactionAdded")
        }
    }
    ...
    func save() {
        ...
        do {
            ...
            // Don't reload widgets on failure: the persisted store is unchanged, so refreshing
            // them would surface stale-but-presented-as-current data. Surface the error to the
            // unified log instead of swallowing it silently.
            DataController.logger.error("Failed to save Core Data context: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    func updateRecurringTransaction(transaction: Transaction) {
        if transaction.nextTransactionDate < Calendar.current.startOfDay(for: Date.now) {
            ...
            while holdingDate <= Calendar.current.startOfDay(for: Date.now) {
                ...
                newTransaction.month = calendar.date(from: dateComponents) ?? Date.now
                ...
                if transaction.recurringType == 1 {
                    newDate = Calendar.current.date(byAdding: .day, value: Int(transaction.recurringCoefficient), to: holdingDate) ?? holdingDate
                } else if transaction.recurringType == 2 {
                    newDate = Calendar.current.date(byAdding: .day, value: Int(transaction.recurringCoefficient * 7), to: holdingDate) ?? holdingDate
                } else if transaction.recurringType == 3 {
                    newDate = Calendar.current.date(byAdding: .month, value: Int(transaction.recurringCoefficient), to: holdingDate) ?? holdingDate
                }
